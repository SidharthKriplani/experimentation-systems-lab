...
- **Foundation module depth audit — RCA, Metrics, Exp (audit #96)** — three rooms are significantly thinner than Stat Foundations (25 modules): Exp=7, Metrics=8, RCA=6. Assess what's covered, what's missing, and whether additional modules are justified by interview frequency. Do not add padding. ~1 session per room. Gate: fix audit #95 first.

### Bugs
- **React error boundary — missing entirely (audit #105)** — no `<ErrorBoundary>` exists anywhere in the codebase. Three confirmed white-screen crashes this session cycle (Behavioral room, Cases shuffling, Module25_IV null deref). Fix: create `src/components/shared/ErrorBoundary.jsx` (class component), wrap `<main>` in `App.jsx`. Show "Something went wrong — go home" fallback with a CTA instead of blank screen. One component, ~30 min. Fixes real user pain and adds engineering signal for any reviewer who runs the app.
- **Data file validator script (audit #102)** — zero test coverage in the repo. Two production builds have already been broken by apostrophe/template literal bugs in data files. Fix: `scripts/validate-data.js` — Node script that imports every `src/data/*.js` file and checks for illegal syntax (backticks outside comments, unescaped apostrophes in single-quoted strings), missing required fields (`id`, `title`, `difficulty`), and empty arrays. Wire as `npm run validate-data`. No test framework needed — pure Node. Run before every commit that touches `src/data/`. Effort: ~1 session. Highest-value test work relative to PAL's actual recurring pain points.
- **Foundation module subtitle duplication bug (audit #94)** — subtitle renders in the yellow runner header card (correct, intended) AND as the first words of the module body text (wrong, no separator). Verified in Stat Foundations Module 02 (Mean/Median/Mode) and Module 04 (Normal Distribution). Assumed same pattern in Exp, Metrics, RCA module files — verify before bulk editing. Fix: read `Module02_MeanMedianMode.jsx` first to determine whether the subtitle text is (a) hardcoded at the start of the body explanation string, or (b) rendered as a separate `{subtitle}` JSX element before the body paragraph. Remove whichever source is duplicating, leaving only the substantive explanation. Verify render in one module, then apply confirmed pattern to all module files across all four foundation rooms. Files: `src/components/[foundation]/modules/*.jsx`. Effort: ~30–45 min once root cause is confirmed.
...
- **Homepage framing pass (audit #103)** — README corrected in V4.33.7 (audience, localStorage/Supabase contradiction, canonical product framing). The live homepage (`src/pages/Home.jsx`) was not audited in the same pass. A cold visitor hits the homepage first. Check for: "Data Scientist" anywhere in hero or room cards, "no backend" or "localStorage only" framing, sign-in CTA copy accuracy, any copy misaligned with "interactive judgment system for product analysts, data analysts, and PMs." Copy-only pass, no component logic. Effort: ~20 min.
